...
func (mq *MachineQuery) QueryIps() *IPQuery {
	query := &IPQuery{config: mq.config}
...
func (mq *MachineQuery) QueryUsers() *UserQuery {
...
func (mq *MachineQuery) QueryKeys() *KeyQuery {
...
func (mq *MachineQuery) Clone() *MachineQuery {
	...
	return &MachineQuery{
		...
		withIps:    mq.withIps.Clone(),
		withUsers:  mq.withUsers.Clone(),
		withKeys:   mq.withKeys.Clone(),
...
func (mq *MachineQuery) WithIps(opts ...func(*IPQuery)) *MachineQuery {
	query := &IPQuery{config: mq.config}
...
func (mq *MachineQuery) WithUsers(opts ...func(*UserQuery)) *MachineQuery {
	query := &UserQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MachineQuery) WithKeys(opts ...func(*KeyQuery)) *MachineQuery {
	query := &KeyQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MachineQuery) sqlAll(ctx context.Context) ([]*Machine, error) {
	...
	if query := mq.withIps; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Machine)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Ips = []*IP{}
		}
		query.withFKs = true
		query.Where(predicate.IP(func(s *sql.Selector) {
			s.Where(sql.InValues(machine.IpsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.ip_machine
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "ip_machine" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "ip_machine" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Ips = append(node.Edges.Ips, n)
		}
	}

	if query := mq.withUsers; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*Machine, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(machine.UsersPrimaryKey[1], fks...))
			},
	...
	if query := mq.withKeys; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Machine)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Keys = []*Key{}
		}
		query.withFKs = true
		query.Where(predicate.Key(func(s *sql.Selector) {
			s.Where(sql.InValues(machine.KeysColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.key_machine
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "key_machine" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "key_machine" returned %v for node %v`, *fk, n.ID)
			}
