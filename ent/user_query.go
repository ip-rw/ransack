...
func (uq *UserQuery) QueryMachines() *MachineQuery {
	query := &MachineQuery{config: uq.config}
...
func (uq *UserQuery) QueryKeys() *KeyQuery {
	query := &KeyQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withMachines: uq.withMachines.Clone(),
...
func (uq *UserQuery) WithMachines(opts ...func(*MachineQuery)) *UserQuery {
	query := &MachineQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withMachines = query
	return uq
}
...
func (uq *UserQuery) WithKeys(opts ...func(*KeyQuery)) *UserQuery {
	query := &KeyQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withMachines; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Machines = []*Machine{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.MachinesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "machines": %w`, err)
		}
		query.Where(machine.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "machines" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Machines = append(nodes[i].Edges.Machines, n)
			}
		}
	}
	...
	if query := uq.withKeys; query != nil {
		...
		for _, n := range neighbors {
			fk := n.key_user
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "key_user" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "key_user" returned %v for node %v`, *fk, n.ID)
			}
