...
func (uc *UserCreate) AddMachineIDs(ids ...int) *UserCreate {
	uc.mutation.AddMachineIDs(ids...)
	return uc
}

// AddMachines adds the "machines" edges to the Machine entity.
func (uc *UserCreate) AddMachines(m ...*Machine) *UserCreate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return uc.AddMachineIDs(ids...)
}
...
func (uc *UserCreate) AddKeys(k ...*Key) *UserCreate {
	ids := make([]int, len(k))
	for i := range k {
		ids[i] = k[i].ID
	}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.MachinesIDs(); len(nodes) > 0 {
