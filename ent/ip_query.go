...
func (iq *IPQuery) QueryMachine() *MachineQuery {
	query := &MachineQuery{config: iq.config}
...
func (iq *IPQuery) FirstIDX(ctx context.Context) int {
...
// Returns a *NotSingularError when exactly one IP entity is not found.
...
// Returns a *NotSingularError when exactly one IP ID is not found.
...
func (iq *IPQuery) OnlyIDX(ctx context.Context) int {
...
func (iq *IPQuery) Clone() *IPQuery {
	...
	return &IPQuery{
		...
		withMachine: iq.withMachine.Clone(),
...
func (iq *IPQuery) WithMachine(opts ...func(*MachineQuery)) *IPQuery {
	query := &MachineQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withMachine = query
	return iq
}
...
func (iq *IPQuery) sqlAll(ctx context.Context) ([]*IP, error) {
	...
	if iq.withMachine != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, ip.ForeignKeys...)
	}
	...
	if query := iq.withMachine; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*IP)
		for i := range nodes {
			fk := nodes[i].ip_machine
			if fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "ip_machine" returned %v`, n.ID)
			}
			for i := range nodes {
...
func (iq *IPQuery) sqlQuery(ctx context.Context) *sql.Selector {
...
func (is *IPSelect) Scan(ctx context.Context, v interface{}) error {
	...
	is.sql = is.IPQuery.sqlQuery(ctx)
